...
def _dummy_dataset_with_images(
  ...
  return grain.MapDataset.source(source).map(
      lambda x: vl_dpo_lib.TrainingInput(
...
class VlDpoTrainerTest(parameterized.TestCase):
  ...
  def test_vl_dpo_trainer(
    ...
    dpo_config = vl_dpo_lib.DpoTrainingConfig(
        eval_every_n_steps=10,
        max_steps=10,
    )
    vl_dpo_trainer = vl_dpo_lib.DpoTrainer(
        model=model,
        ref_model=ref_model,
  ...
  def test_vl_dpo_loss_fn(self):
    ...
    train_example = vl_dpo_lib.TrainExample(
        ...
        images=jnp.ones((8, 3, 224, 224)),  # Example image embeddings
    )

    with mock.patch.object(
        common, "get_per_token_logps", return_value=jnp.array(per_token_logps)
    ):
      loss, _ = vl_dpo_lib.dpo_loss_fn(model, train_example, 0.1, 0)
      np.testing.assert_allclose(loss, 0.753059, atol=1e-5)

      loss, _ = vl_dpo_lib.dpo_loss_fn(model, train_example, 0.1, 0.3)
  ...
  def test_vl_dpo_prepare_inputs(self):
    ...
    vl_dpo_trainer = vl_dpo_lib.DpoTrainer(
        ...
        ref_model=ref_model,
        optimizer=optax.sgd(1e-3),
        training_config=vl_dpo_lib.DpoTrainingConfig(
            eval_every_n_steps=10,
            max_steps=10,
        ),
    )

    training_input = vl_dpo_lib.TrainingInput(
    ...
    self.assertEqual(
        out.images.shape, (4, 3, 224, 224)
    )  # Check image duplication

  def test_vl_dpo_vs_dpo_no_images(self):
    """Compare VL_DPOTrainer and DPOTrainer when no images are provided."""
    model = tc.ToyTransformer(rngs=nnx.Rngs(0))
    ref_model = tc.ToyTransformer(rngs=nnx.Rngs(0))
